...
async def on_ready():
    ...
    await client.send_message(client.get_channel('*'), "Good Morning Master! :kissing_closed_eyes:")
...
async def bo3(ctx, t, l):
    ...
    await client.send_message(client.get_channel('*'), "__**New battle request has "
                                                                        "been posted <@&537315150009466880>!**__\n")
    await client.send_message(client.get_channel('*'),
                              "**Player:** " + ctx.message.author.display_name)
    await client.send_message(client.get_channel('*'), "**Availability:** " + t + " " + time)
    await client.send_message(client.get_channel('*'), "**Battle Type:** Best of 3")
    await client.send_message(client.get_channel('*'), "__**Please DM " + ctx.message.author.mention +
...
async def bo5(ctx, t, l):
    ...
    await client.send_message(client.get_channel('537750598662225940'),
                              "**Player:** " + ctx.message.author.display_name)
    await client.send_message(client.get_channel('*'), "**Availability:** " + t + " " + time)
    await client.send_message(client.get_channel('*'), "**Battle Type:** Best of 5")
    await client.send_message(client.get_channel('*'), "__**Please DM " + ctx.message.author.mention +
                              " for more info or friend link**__")
...
async def mute(ctx, user: discord.Member):
    ...
    for roles in user.roles:
        if "@everyone" in str(roles):
            ...
            await client.send_message(client.get_channel('*'), "```" + str(roles) + "```")
    await client.replace_roles(user, role)
    await client.send_message(client.get_channel('*'),
                              "**" + user.display_name + " was muted from all chat on __" + d.strftime(
                                  "%d-%B-%Y %H:%M") + "__**")
    await client.send_message(client.get_channel('*'), 'Hello ' + user.mention
